...
func (t *SaveFoundationTool) Execute(_ context.Context, args json.RawMessage) (json.RawMessage, error) {
	...
	if volumeEnd && strings.TrimSpace(a.Reason) == "" {
		return nil, fmt.Errorf("%s requires reason parameter: check completion checklist, state in one sentence why continuing, wrapping up, or completing: %w", a.Type, errs.ErrToolArgs)
	}
	...
	case "append_volume":
		...
		if p != nil && p.Phase == domain.PhaseComplete {
			return nil, fmt.Errorf("book is completed (phase=complete), cannot append new volume: %w", errs.ErrToolPrecondition)
		}
	...
	case "complete_book":
		...
		if progress.Phase != domain.PhaseWriting {
			return nil, fmt.Errorf("complete_book only allowed in writing phase (current phase=%s): %w", progress.Phase, errs.ErrToolPrecondition)
		}
		if len(progress.PendingRewrites) > 0 {
			return nil, fmt.Errorf("there are %d chapters in pending rewrites, complete them before calling complete_book: %w", len(progress.PendingRewrites), errs.ErrToolPrecondition)
		}
		...
		if len(progress.CompletedChapters) == 0 {
			return nil, fmt.Errorf("cannot complete book without writing a single chapter; system automatically advances writing after planning, no need to call complete_book: %w", errs.ErrToolPrecondition)
		}
		...
		if progress.Layered {
			...
			if next <= len(outline) {
				return nil, fmt.Errorf("detailed outline has unwritten chapters (next %d/refined %d), cannot complete book; to wrap up early use append_volume with \"final\": true at volume JSON top level: %w", next, len(outline), errs.ErrToolPrecondition)
			}
		} else if progress.TotalChapters > 0 && next <= progress.TotalChapters {
			return nil, fmt.Errorf("outline has unwritten chapters (next %d/total %d), cannot complete book; to wrap up early use append_volume with \"final\": true at volume JSON top level: %w", next, progress.TotalChapters, errs.ErrToolPrecondition)
		}
		...
		if compass != nil && len(compass.OpenThreads) > 0 {
			return nil, fmt.Errorf("compass has %d active open threads (e.g.: %s), cannot complete book. update_compass to clear open_threads if all resolved, or append_volume (with \"final\": true to declare wrap-up): %w",
