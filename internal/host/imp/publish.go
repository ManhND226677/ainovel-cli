...
func publishFoundation(st *store.Store, f *Foundation) error {
	...
	if p == nil {
		return fmt.Errorf("load progress: progress is missing")
	}
...
func checkFoundationConflicts(st *store.Store, f *Foundation) error {
	...
	if err != nil {
		return fmt.Errorf("read final premise: %w", err)
	}
	if cur != "" && cur != f.Premise {
		return fmt.Errorf("final premise conflicts with import synthesis (different version exists), refusing to overwrite")
	}
	chars, err := st.Characters.Load()
	if err != nil {
		return fmt.Errorf("read final characters: %w", err)
	}
	if len(chars) > 0 && !jsonEqual(chars, f.Characters) {
		return fmt.Errorf("final characters conflict with import synthesis (different version exists), refusing to overwrite")
	}
	rules, err := st.World.LoadWorldRules()
	if err != nil {
		return fmt.Errorf("read final world_rules: %w", err)
	}
	if len(rules) > 0 && !jsonEqual(rules, f.WorldRules) {
		return fmt.Errorf("final world_rules conflict with import synthesis (different version exists), refusing to overwrite")
	}
	layered, err := st.Outline.LoadLayeredOutline()
	if err != nil {
		return fmt.Errorf("read final layered_outline: %w", err)
	}
	if len(layered) > 0 && !jsonEqual(layered, f.Volumes) {
		return fmt.Errorf("final layered_outline conflicts with import synthesis (different version exists), refusing to overwrite")
	}
	return nil
}
...
func isPublished(st *store.Store, expected int) (bool, error) {
	...
	if err != nil {
		return false, fmt.Errorf("read final premise: %w", err)
	}
	if p == "" {
		return false, nil
	}
	...
	if err != nil {
		return false, fmt.Errorf("read final outline: %w", err)
	}
	if len(o) < expected {
		return false, nil
	}
	...
	if err != nil {
		return false, fmt.Errorf("read final progress: %w", err)
	}
