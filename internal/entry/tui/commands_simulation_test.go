...
func TestSimulationCommandsAreRegisteredAndNeedIdle(t *testing.T) {
	...
	for _, name := range []string{"mo-phong", "nhap-mo-phong", "simulate", "importsim"} {
	...
	if !hasPaletteItem(items, "mo-phong") || !hasPaletteItem(items, "nhap-mo-phong") {
		t.Fatalf("expected Vietnamese simulate commands in palette: %+v", items)
	}
}
...
func TestSimulationCommandsAreBlockedWhileRunning(t *testing.T) {
	...
	next, _ := m.handleSlashCommand(slashCommand{name: "mo-phong"})
	got := next.(Model)
